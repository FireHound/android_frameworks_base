...
public class GamingModeTile extends QSTileImpl<BooleanState> {
    ...
    private void handleState(boolean enabled) {
        boolean headsUpEnabled = Settings.Global.getIntForUser(mContext.getContentResolver(),
                Settings.Global.HEADS_UP_NOTIFICATIONS_ENABLE, 1, UserHandle.USER_CURRENT);
        if (enabled && headsUpEnabled) {
            Settings.Global.putIntForUser(mContext.getContentResolver(),
                    Settings.Global.HEADS_UP_NOTIFICATIONS_ENABLE, 0, UserHandle.USER_CURRENT);
        } else if (!enabled) {
            Settings.Global.putIntForUser(mContext.getContentResolver(),
                        Settings.Global.HEADS_UP_NOTIFICATIONS_ENABLE, 1, UserHandle.USER_CURRENT);
        }
        boolean isHwKeysOn =  Settings.Secure.getIntForUser(mContext.getContentResolver(),
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        final boolean gamingMode = value != 0;
        state.value = enable;
        state.label = mContext.getString(R.string.gaming_mode_title);
        if (enable) {
            state.icon = ResourceIcon.get(R.drawable.ic_gaming_mode_on);
            state.contentDescription =  mContext.getString(
                    R.string.accessibility_quick_settings_gaming_mode_on);
            state.state = Tile.STATE_ACTIVE;
        } else {
            state.icon = ResourceIcon.get(R.drawable.ic_gaming_mode_off);
            state.contentDescription =  mContext.getString(
    ...
    public CharSequence getTileLabel() {
        return mContext.getString(R.string.gaming_mode_title);
    }
